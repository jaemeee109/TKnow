    ...
    public ResponseEntity<MemberResponseDTO> getMyInfo(Principal principal,
        ...
        try {
            if (principal == null || principal.getName() == null) {
                log.warn("로그인 정보 없음 | X-Request-Id={}", requestId);
                return ResponseEntity.status(401).build(); // Unauthorized
            }

            String memberId = principal.getName();
            log.info("[GET] /members/me | X-Request-Id={} | memberId={}", requestId, memberId);
            ...
            log.error("회원 조회 중 서버 오류 발생 | X-Request-Id={}", requestId, e);
